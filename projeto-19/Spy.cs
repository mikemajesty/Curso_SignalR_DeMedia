using Microsoft.AspNet.SignalR;
using Microsoft.Owin;
using System;
using System.Threading.Tasks;

namespace projeto_19
{
    public class Spy : OwinMiddleware
    {
        private static IPersistentConnectionContext connSpy = GlobalHost.ConnectionManager.GetConnectionContext<ConnectionSpy>();
        public Spy(OwinMiddleware next) : base(next)
        {

        }
        public override Task Invoke(IOwinContext context)
        {
            var message = string.Format("{0}: Request {1} from IP {2} using {3}",
                DateTime.Now.ToShortDateString(), context.Request.Uri.ToString(), context.Request.Host, context.Request.Headers["USER-AGENT"]);
            return Next.Invoke(context).ContinueWith(c => connSpy.Connection.Broadcast(message));
        }
    }
}